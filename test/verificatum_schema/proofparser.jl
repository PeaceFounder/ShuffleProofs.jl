...
using Test
import SigmaProofs.Parser: decode, convert, unmarshal_publickey, interpret, Tree, encode, Leaf, Leaf, unmarshal
import CryptoGroups: PGroup, value, order, CryptoGroups
import CryptoPRG: HashSpec
import CryptoPRG.Verificatum: RO, PRG
import SigmaProofs.ElGamal: ElGamalRow
import SigmaProofs.Serializer: load
import SigmaProofs.Verificatum: ProtocolSpec, ro_prefix
...
protinfo = load(ProtocolSpec, PROT_INFO)
...
(; g, nr, nv, ne, prghash, rohash) = protinfo
G = typeof(g)

pk_tree = decode(read(FULL_PUBLIC_KEY))
_, pk = convert(Tuple{G, G}, pk_tree)
...
𝐡 = generator_basis(protinfo, G, N)
